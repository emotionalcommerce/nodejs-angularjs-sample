...
var content = $('<div class="timepicker">\
		<div v-show class="title"></div>\
			<div class="chose-all">\
				...
				<div class="text">\
					<div class="cell-4"><a class="js-hour-show" title="pick hour"></a></div>\
					<div class="cell-2">:</div>\
					<div class="cell-4"><a class="js-minute-show" title="pick hour"></a></div>\
				</div>\
				<div class="handle">\
...
timepicker.update = function () {
    ...
    var t=this.inputTarget.scope();
    if(this.inputTarget.type=="start") {
    	if(t.day) {
            t.day.item.start_hour = this.inputTarget[0].value;
            t.day.item.non_stop = false;
            t.day.item.closed = false;
            if(t.day.item.title=='Monday') {
           		var x = document.getElementsByClassName('automat');
				for(var k=0; k<x.length; k++) {
					angular.element(x[k]).val(this.inputTarget[0].value);
					angular.element(x[k]).trigger('input');
					angular.element(x[k]).trigger('change');

				}
            }

		}
    	if(t.holiday) t.holiday.item.start_hour = this.inputTarget[0].value;
    }
    if(this.inputTarget.type=="end") {
        if(t.day) {
            t.day.item.end_hour = this.inputTarget[0].value;
            t.day.item.non_stop = false;
            t.day.item.closed = false;
            if(t.day.item.title=='Monday') {
                var x = document.getElementsByClassName('automat_end');
                for (var k = 0; k < x.length; k++) {
                    angular.element(x[k]).val(this.inputTarget[0].value);
                    angular.element(x[k]).trigger('input');
                    angular.element(x[k]).trigger('change');
    	...
    	if(t.holiday) t.holiday.item.end_hour = this.inputTarget[0].value;
    }
	return this;
};
